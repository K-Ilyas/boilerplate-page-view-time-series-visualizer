...
def draw_bar_plot():
    ...
    df_bar = df.copy()
    df_bar = df_bar.groupby(pd.Grouper(freq="M"),sort=False)
    df_bar = df_bar.mean().round()
    
    df_bar.reset_index(inplace=True, level=['date'])
    df_bar['Years'] =  df_bar['date'].dt.strftime('%Y').astype(int)
